...
# =============================================================================
# Crew Ops Agent-a-thon — Infrastructure Deployment Script
# Provisions: AI Foundry (account + project + model), Log Analytics, App Insights
...
RESOURCE_GROUP="${RESOURCE_GROUP:-crewops-agentathon-rg-$SUFFIX}"
LOCATION="${LOCATION:-swedencentral}"
FOUNDRY_RESOURCE_NAME="${FOUNDRY_RESOURCE_NAME:-crewops-foundry-$SUFFIX}"
PROJECT_NAME="${PROJECT_NAME:-crewops-project}"
MODEL_DEPLOYMENT_NAME="${MODEL_DEPLOYMENT_NAME:-gpt-5.4}"
MODEL_NAME="${MODEL_NAME:-gpt-5.4}"
MODEL_VERSION="${MODEL_VERSION:-2026-03-05}"
LOG_ANALYTICS_NAME="${LOG_ANALYTICS_NAME:-crewops-logs-$SUFFIX}"
APP_INSIGHTS_NAME="${APP_INSIGHTS_NAME:-crewops-insights-$SUFFIX}"

# --- Argument parsing --------------------------------------------------------
TAGS=("environment=agentathon")
while [[ $# -gt 0 ]]; do
    case "$1" in
        --tags)
            shift
...
echo "=============================================="
echo "  Crew Ops Agent-a-thon — Infrastructure Deploy"
echo "=============================================="
echo ""
echo "Suffix:            $SUFFIX"
...
if ! az rest \
    ...
    --output none; then
    echo "Could not link Application Insights to the account automatically."
...
cat > "$ENV_FILE" << EOF
# =============================================================================
# Crew Ops Agent-a-thon — Environment Variables
...
# Optional: a second, cheaper deployment for the Triage Agent. Leave unset to
# reuse MODEL_DEPLOYMENT_NAME for all three agents.
...
# Challenge 4 — Portal Workflow (set after creating the workflow in Foundry portal)
# Build -> Workflows -> New workflow -> deploy -> copy the agent name here
WORKFLOW_AGENT_NAME=
EOF
