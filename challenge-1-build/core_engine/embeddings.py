...
scale, reused by `rule_search.py` and `intent_search.py` instead of being
...
def embed_query(text: str) -> list[float]:
    ...
    stored in this project's *_vec tables. `text-embedding-3-small` supports
    requesting a shorter output directly, so a fresh query is comparable to
    rows this project's original system already embedded, with nothing here
    ...
    token as `api_key`. `AzureOpenAI(azure_endpoint=...)` would double-append
...
def _is_flat(scores: list[float]) -> bool:
    ...
    share literally every word with the text. That's the common case, not
    an edge case, for the paraphrased questions this search exists to
    handle at all."""
...
def blend(bm25_scores: list[float], semantic_scores: list[float], alpha: float) -> list[float]:
    ...
    [0, 1] — except when one side is flat (see `_is_flat`). Blending a flat,
    uninformative side in at its full weight would silently cap every
    achievable score at `alpha` (or `1 - alpha`), no matter how strong the
    other, actually-informative side is. A real paraphrase with a perfect
    semantic match but zero keyword overlap would then be mathematically
    unable to ever clear a threshold above 0.5. So a flat side is dropped
    instead, and the informative side carries the blend alone, avoiding
    that silent, structural ceiling.
    """
